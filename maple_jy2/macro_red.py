...
PATROL_LEARN_THRESHOLD  = 50    # mm 범위가 이 값 이상이어야 위치 기반 방향 활성
PATROL_SETUP_SECONDS    = 50    # 처음 N초는 시간 기반 반전만 (학습 더 길게)
ATTACK_DOWN_SECONDS     = 0.06  # 공격 키 누름 시간

# ============================================================
# 사용자 측정값 — 측정 후 여기만 수정하면 됨
# ============================================================

# 1) 안전지대 / 포탈 / 위 텔포 위치 (mm_probe_red.py로 측정)
SAFE_ZONE_X_LEFT_MARGIN  = 8    # 클릭 X 기준 왼쪽 허용 여유 (미니맵 픽셀)
SAFE_ZONE_X_RIGHT_MARGIN = 3    # 클릭 X 기준 오른쪽 허용 여유 (위 텔포 가능 영역 살짝만)
SAFE_ZONE_TARGET_Y       = 83   # 안전지대 도착 판정 mm_y (이 값 이하면 도착)
SAFE_ZONE_X_FULL         = (174, 202)  # 참고용 — 안전지대 전체
PORTAL_DANGER_X          = (180, 192)  # 포탈 영역 — 위 텔포 시전 금지
SAFE_ZONE_TP_X           = (193, 202)  # 실제 위 텔포 시전 영역 (안전지대 우측, 포탈 회피)

# 2) 인벤토리 / 휴대용 상점 UI 좌표
# ui_probe.py 실행 후 생성되는 ui_probe_params.json 이 있으면 아래 기본값은 자동으로 덮어써짐.
# 수동 입력이 필요하면 ui_probe.py 출력 python 블록을 이 구간에 붙여넣어도 됨.
# === BEGIN UI_PROBE_PARAMS ===
COORDS_CAL_GAME_LEFT = -11     # 측정 시점 게임창 left
COORDS_CAL_GAME_TOP  = 0       # 측정 시점 게임창 top
COORDS_CAL_GAME_W    = 3126    # 측정 시점 게임창 width
COORDS_CAL_GAME_H    = 1758    # 측정 시점 게임창 height
CASH_TAB_ABS         = (1740, 119)
PORTABLE_SHOP_ABS    = (1684, 511)
SHOP_FIRST_SLOT_ABS  = (1023, 532)   # 상점 UI 등장 검증용 (열림 여부 std 체크)
SELL_ALL_BUTTON_ABS  = (1351, 297)   # 장비 일괄 판매 버튼
SHOP_CLOSE_ABS       = (895, 298)
EQUIP_TAB_ABS        = (1473, 120)
ETC_TAB_ABS          = (1289, 461)   # 기타 탭 (상점 열린 상태에서 측정)
ETC_SELL_SLOT_ABS    = (1296, 526)   # 기타템 판매 — 인벤 첫 슬롯 1회 클릭 → 첫 아이템 선택
ETC_SELL_BUTTON_ABS  = (1348, 338)   # "팔기" 버튼 — 클릭 후 ENTER 반복
ETC_CHECK_SLOT_ABS   = (1015, 522)   # 인벤 기타 첫 칸 — std 체크용 (빈칸=다 팔렸음)
INV_FIRST_SLOT_ABS   = (1482, 186)
INV_TRIGGER_ABS      = (1664, 489, 1718, 541)  # x1, y1, x2, y2
# === END UI_PROBE_PARAMS ===

# 3) 사냥 라인 / 층 / 회수 루트 미니맵 좌표 (mm_probe_red.py로 측정)
# - 미니맵 기준: X는 왼쪽→오른쪽으로 증가, Y는 위쪽으로 갈수록 작아짐.
# - 맵 기준: 1층=배 하단 내부/아래 긴 발판, 2층=갑판 사냥 라인, 3층=돛대 주변 상단 발판.
# - X 범위는 캐릭터 점이 그 범위 안에 들어오면 "도착"으로 보는 여유 구간.
PATROL_LEFT_BOUND  = 74   # 2층 갑판 사냥 왼쪽 끝. 도달하면 오른쪽으로 반전.
PATROL_RIGHT_BOUND = 191  # 2층 갑판 사냥 오른쪽 끝. 도달하면 왼쪽으로 반전.
HUNT_X_MIN = 142          # 2층 우측 위 platform/z 회수 구간의 좌측 경계.
HUNT_X_MAX = 151          # 2층 우측 위 platform/z 회수 구간의 우측 경계.

FLOOR1_Y       = 119      # 1층: 배 하단 내부/아래 긴 발판의 기준 Y.
FLOOR2_Y       = 100      # 2층: 주 사냥 라인(갑판)의 기준 Y.
FLOOR2_STAIR_Y = 91       # 2층과 3층 사이 작은 계단/중간 발판 Y.
FLOOR3_Y       = 82       # 3층: 돛대 주변 상단 발판의 기준 Y.
FLOOR3_TOP_Y   = 72       # 3층에서 위+텔포 후 닿는 더 높은 상단 발판 Y.
FLOOR2_RIGHT_TOP_Y = 89   # 2층 오른쪽에서 위+텔포/점프 후 닿는 우측 상단 발판 Y.
COLLECT_TOP_PLATFORM_Y = 78  # 회수 루트 중 hold_up_teleport 후 목표로 보는 상단 Y.

ROUTE_F1_UPTP_X      = (59, 67)    # 1층 왼쪽 계단. 1층→2층 복귀 위+텔포 시도 위치.
ROUTE_F2_STAIR_X     = (92, 99)    # 2층 중간 계단. 점프 후 2층계단 Y 진입 확인 구간.
ROUTE_F3_X           = (108, 117)  # 3층 중앙 회수 구간. z hold 왕복으로 줍는 범위.
ROUTE_F2_RIGHT_X     = (142, 151)  # 2층 오른쪽 위 platform 회수 구간. HUNT_X와 같은 위치.

COLLECT_F1_PATROL_X = (119, 223)   # 1층 하단을 좌우로 훑는 텔포공격 왕복 범위.
COLLECT_F1_UPTP_X   = (59, 67)     # 1층에서 2층으로 올라가기 위한 위+텔포 시작 범위.
COLLECT_F1_CLEAR_X  = (59, 78)     # 위+텔포 전 1층 왼쪽 계단 주변 몹 정리 범위.
COLLECT_F1_LEFT_STOP_X = 150       # 1층 진입 후 먼저 맞추는 중간 X. 좌측 회수 동선 시작점.

ROUTE_3F_ENTRY_X        = 114          # 2층에서 3층 루트로 진입할 위+텔포 시작 X.
ROUTE_3F_TOP_Y          = 64           # 3층 진입 중 hold_up_teleport 후 기대하는 최상단 Y.
ROUTE_3F_PLATFORM_X     = (108, 117)   # 3층 발판 위 z bounce 회수 X 범위.
ROUTE_3F_PLATFORM_Y     = 72           # 3층 발판 위 z bounce 회수 Y.
ROUTE_3F_DROP_TARGET_X  = 101          # 3층에서 2층계단 쪽으로 내려가기 전 맞추는 X.
ROUTE_3F_DROP_TARGET_Y  = 82           # 3층→2층계단 낙하/이동 중 목표 Y.
ROUTE_F2_STAIR_Y        = 91           # 2층계단 z bounce 회수 Y. FLOOR2_STAIR_Y와 같은 기준.
...
GOTO_SAFE_MAX_TRIES      = 25   # up+tp 최대 시도 (mm_y=83 도달까지 반복)
GOTO_SAFE_TP_TRIGGER_DX  = 5    # 이 이상 멀면 텔포, 미만이면 walk — 거의 항상 텔포 (몹 넉백 회피)
GOTO_SAFE_X_ARRIVAL_TOL  = 3    # X 범위 밖이라도 이만큼 가까우면 도착 인정 (텔포 overshoot 허용)
...
_UI_PROBE_INT_PARAMS = {
    'COORDS_CAL_GAME_LEFT',
    'COORDS_CAL_GAME_TOP',
    'COORDS_CAL_GAME_W',
    'COORDS_CAL_GAME_H',
}

_UI_PROBE_TUPLE_PARAMS = {
    'CASH_TAB_ABS': 2,
    'PORTABLE_SHOP_ABS': 2,
    'SHOP_FIRST_SLOT_ABS': 2,
    'SELL_ALL_BUTTON_ABS': 2,
    'SHOP_CLOSE_ABS': 2,
    'EQUIP_TAB_ABS': 2,
    'ETC_TAB_ABS': 2,
    'ETC_SELL_SLOT_ABS': 2,
    'ETC_SELL_BUTTON_ABS': 2,
    'ETC_CHECK_SLOT_ABS': 2,
    'INV_FIRST_SLOT_ABS': 2,
    'INV_TRIGGER_ABS': 4,
}


def _apply_ui_probe_config(path: str = UI_PROBE_CONFIG_PATH) -> bool:
    """ui_probe.py가 저장한 좌표 파일이 있으면 기본 UI 좌표를 덮어쓴다."""
    ...
    try:
        ...
        print(f'[ui-probe] 설정 로드 실패: {path} ({e})')
        return False

    params = payload.get('params', payload)
    if not isinstance(params, dict):
        print(f'[ui-probe] 설정 형식 오류: params가 dict가 아님 ({path})')
        return False

    updates = {}
    for name in _UI_PROBE_INT_PARAMS:
        if name not in params:
            continue
        try:
            updates[name] = int(params[name])
        except (TypeError, ValueError):
            print(f'[ui-probe] {name} 값 무시: {params[name]!r}')

    for name, expected_len in _UI_PROBE_TUPLE_PARAMS.items():
        if name not in params:
            continue
        value = params[name]
        if not isinstance(value, (list, tuple)) or len(value) != expected_len:
            print(f'[ui-probe] {name} 값 무시: {value!r}')
            continue
        try:
            updates[name] = tuple(int(v) for v in value)
        except (TypeError, ValueError):
            print(f'[ui-probe] {name} 값 무시: {value!r}')

    if updates:
        globals().update(updates)
        print(f'[ui-probe] {path} 적용: {len(updates)}개 파라미터')
        return True

    print(f'[ui-probe] 적용할 파라미터 없음: {path}')
    return False


_apply_ui_probe_config()
...
LIE_THRESHOLD     = 0.70   # 새 해상도 안티얼라이싱 차이 → 임계값 완화 (옛 0.85)
LIE_DIAG_THR      = 0.55   # 이 점수 이상이면 디버그 화면 자동 저장 (마을 false positive ~0.43 차단)
LIE_DIAG_INTERVAL = 10.0   # 같은 의심 점수 N초 안 중복 저장 방지
...
CON_TEMPLATE          = 'captcha_assets/con_.png.png'
CON_THRESHOLD         = 0.55   # 안티얼라이싱 차이 → 임계값 완화 (옛 0.80)
CON_CHECK_EVERY       = 16
CON_CHECK_WINDOW_SEC  = 30.0  # 캡차 풀이 후 N초간만 polling 활성
CON_MAX_TRIES         = 6     # ENTER 시도 횟수 — 움직임 테스트로 안전 break 있어서 폭주 안 함
...
TELEPORT_TRIGGER_DX = int(TELEPORT_TRIGGER_DX * _SCALE_X)
ARRIVAL_DEADZONE    = int(ARRIVAL_DEADZONE    * _SCALE_X)
...
def _coord_scale() -> tuple[float, float]:
    """ui_probe 측정 당시 게임창 크기 대비 현재 게임창 크기 비율."""
    sx = GAME_REGION['width'] / COORDS_CAL_GAME_W if COORDS_CAL_GAME_W else 1.0
    sy = GAME_REGION['height'] / COORDS_CAL_GAME_H if COORDS_CAL_GAME_H else 1.0
    return sx, sy


def _abs(x: int, y: int) -> tuple:
    """ui_probe 절대 좌표 → 현재 화면 절대 좌표.
    측정 당시 GAME_REGION left/top을 뺀 뒤 현재 게임창 크기 비율로 보정한다."""
    sx, sy = _coord_scale()
    rel_x = x - COORDS_CAL_GAME_LEFT
    rel_y = y - COORDS_CAL_GAME_TOP
    return (
        int(round(GAME_REGION['left'] + rel_x * sx)),
        int(round(GAME_REGION['top'] + rel_y * sy)),
    )


def _game_rel(x: int, y: int) -> tuple:
    """ui_probe 절대 좌표 → 현재 game grab 영역 내 상대 좌표."""
    sx, sy = _coord_scale()
    rel_x = x - COORDS_CAL_GAME_LEFT
    rel_y = y - COORDS_CAL_GAME_TOP
...
def _shop_check_stop():
    ...
    lie 감지 시 ★상점 먼저 닫고★ 풀이 → _ShopAborted raise.
    상점 UI가 캡차 dialog 뒤에 깔리면 solver의 slider_handle 매칭 score 떨어짐.
    캡차 풀이 후 _shop_captcha_solved=True 세팅 → auto_shop이 자동 재시도."""
    global STOP, _shop_captcha_solved
    if STOP:
    ...
    if LIE_ENABLED and lie_detected(grab()):
        print('[shop] !! 거짓말 탐지기 감지 — 상점 먼저 닫고 풀이')
        attack_release()
        release()
        # 상점 닫기 — 캡차 solver matching에 shop UI 간섭 방지
        # (캡차는 popup이라 background 클릭 가능. 슬라이드 입력만 받아서 다른 클릭 안전)
        sc_x, sc_y = _abs(*SHOP_CLOSE_ABS)
        print(f'[shop] 캡차 풀이 전 상점 닫기 ({sc_x},{sc_y})')
        click_at(sc_x, sc_y)
        rsleep(0.6, 0.2)
        ...
        if not ok:
            ...
            _shop_captcha_solved = True  # 풀이 성공 → auto_shop 재시도 트리거
        raise _ShopAborted()
...
_LIE_DIAG_LAST_TS = 0.0


def lie_detected(screen) -> bool:
    """거짓말 탐지기 매칭. 의심 점수(>=LIE_DIAG_THR) 시 디버그 화면 저장."""
    global _LIE_DIAG_LAST_TS
    if not LIE_ENABLED:
        return False
    # 화면 전체 → 중앙 60%×65% 영역만 매칭
    center = _center_region(screen)
    tpl = _load_tpl(LIE_TEMPLATE)
    if tpl is None:
        return False
    res = cv2.matchTemplate(center, tpl, cv2.TM_CCOEFF_NORMED)
    _, val, _, _ = cv2.minMaxLoc(res)
    # 디버그: 의심 점수면 화면 저장 (중복 방지)
    now = time.time()
    if val >= LIE_DIAG_THR and (now - _LIE_DIAG_LAST_TS) > LIE_DIAG_INTERVAL:
        diag_dir = os.path.join(_LIE_LOG_DIR, 'diag')
        os.makedirs(diag_dir, exist_ok=True)
        ts = datetime.now().strftime('%Y%m%d_%H%M%S')
        path = os.path.join(diag_dir, f'lie_s{val:.2f}_{ts}.png')
        try:
            cv2.imwrite(path, screen)
            print(f'[lie-diag] score={val:.3f} → {path}')
        except Exception as e:
            print(f'[lie-diag] 저장 실패: {e}')
        _LIE_DIAG_LAST_TS = now
    return val >= LIE_THRESHOLD
...
def handle_con_dialog():
    """캡차 통과 안내창 — ENTER 송신.
    매 iter ENTER 치기 직전에 con 재확인 — 이미 닫혔으면 ENTER 안 침 (채팅 켜짐 방지).
    추가로 캐릭 움직임 테스트로 false positive 감지.
    ESC는 절대 안 씀(아이템창 닫힘 방지)."""
    release()
    attack_release()
    print('[*] 캡차 통과 안내창 감지 — ENTER 송신')
    for i in range(CON_MAX_TRIES):
        rsleep(0.4, 0.1)
        # ★ ENTER 직전 재확인 — 다이얼로그 이미 사라졌으면 ENTER 안 침
        if not con_detected(grab()):
            print(f'[+] 안내창 이미 닫힘 (ENTER 전 재확인, {i}회 시도 후)')
            rsleep(0.5, 0.2)
            return True
        tap('enter', 0.05)
        rsleep(0.6, 0.2)
        ...
        if not con_detected(grab()):
            ...
            rsleep(0.8, 0.2)
            return True
        # 2차 종료 조건: 캐릭 움직임 가능 = 다이얼로그 가짜 detection
        pos_before = find_char_minimap_pos(grab())
        tap(RIGHT, 0.10)
        rsleep(0.20, 0.05)
        pos_after = find_char_minimap_pos(grab())
        if (pos_before and pos_after
                and abs(pos_after[0] - pos_before[0]) > 1):
            print(f'[+] 캐릭 움직임 확인 → 다이얼로그 가짜 detection, break ({i + 1}회)')
            rsleep(0.5, 0.2)
            return True
        print(f'[con] {i + 1}/{CON_MAX_TRIES} — con 여전히 감지 + 캐릭 움직임 X')
...
def _handle_lie_detector_core():
    ...
    for attempt in range(1, LIE_MAX_RETRIES + 1):
        ...
        if con_seen:
            ...
            handle_con_dialog()  # ENTER 1회로 닫기
            return True

        # con 매칭 못 잡았어도 lie 다이얼로그가 사라졌으면 풀이는 성공한 것
        # → 안내창 실제 있는지 한 번 더 확인. 있으면 handle_con_dialog. 없으면 ENTER 없이 종료
        # (무조건 ENTER 송신하면 다이얼로그 없을 때 채팅 켜짐)
        if not lie_detected(grab()):
            if con_detected(grab()):
                print('[lie] lie 사라짐 + con 감지 → handle_con_dialog')
                handle_con_dialog()
            else:
                print('[lie] lie/con 모두 없음 → ENTER 없이 종료 (채팅 켜짐 방지)')
                rsleep(0.5, 0.2)
            return True
...
def shop_ui_open() -> bool:
    """
    휴대용 상점 UI가 떠 있는지 검증.
    SHOP_CLOSE 버튼(X) 영역의 std로 판정 — 상점 열리면 X 그래픽 항상 보임,
    빈 슬롯과 무관하게 안정적. (옛 SHOP_FIRST_SLOT 방식은 인벤 비어있으면 실패)
    """
    screen = grab()
    cx, cy = _game_rel(*SHOP_CLOSE_ABS)
    h = SLOT_CHECK_SIZE // 2
    return _region_std(screen, cx - h, cy - h, cx + h, cy + h) > INV_FILLED_STD_THRESHOLD
...
def _shop_sell_etc():
    """기타 탭 전환 후 판매.
    [흐름] 기타탭 → 첫 슬롯 1회 클릭(선택) → '팔기' 버튼+ENTER 반복 → 슬롯 비면 종료.
    다음 아이템은 자동으로 맨 위로 올라와 선택 유지됨 → 슬롯 재클릭 불필요."""
    ...
    try:
        ...
        print(f'[shop-etc] 첫 슬롯 1회 클릭 ({sell_x},{sell_y}) — 첫 아이템 선택')
        click_at(sell_x, sell_y)
        rsleep(0.25, 0.1)             # 선택 안정화
        _shop_check_stop()

        # 팔기 버튼 + ENTER 반복 — 자동으로 다음 아이템이 맨 위로
        sb_x, sb_y = _abs(*ETC_SELL_BUTTON_ABS)
        print(f'[shop-etc] 팔기 버튼 위치 ({sb_x},{sb_y}) — 반복 판매')
        sold = 0
        for i in range(ETC_MAX_SELL_TRIES):
            if STOP:
                break
            if not etc_check_slot_filled(grab()):
                # 빈칸 의심 — 0.4s 대기 후 3회 강력 재검증
                _shop_check_stop()
                rsleep(0.40, 0.1)
                truly_empty = True
                for _ in range(3):
                    if etc_check_slot_filled(grab()):
                        truly_empty = False
                        break
                    rsleep(0.20, 0.1)
                if truly_empty:
                    print(f'[shop-etc] 빈칸 4회 확정 → 기타템 판매 완료 ({sold}개)')
                    break
                # false positive — 다시 시도 (판매 흐름 복귀)
                continue
            click_at(sb_x, sb_y)          # "팔기" 버튼 클릭 (원클릭)
            rsleep(0.08, 0.2)             # 판매 다이얼로그 대기 (이전 더블클릭+ENTER와 동일 타이밍)
            _shop_check_stop()
            tap('enter', 0.03)
            rsleep(0.05, 0.2)             # ENTER 후 갱신 대기
            sold += 1
        else:
        ...
        print('[shop-etc] !! 중단 (캡차/F12) — auto_shop으로 전파')
        raise  # auto_shop이 캡차 재시도 판단


def auto_shop(_retry_after_captcha: bool = True) -> bool:
    """
    ...
    try:
        ...
        # 캡차 풀이로 중단된 거면 한 번 자동 재시도 — 인벤이 가득 찬 상태로 끝나지 않게
        if _shop_captcha_solved and _retry_after_captcha and not STOP:
            _shop_captcha_solved = False
            print('[shop] === 캡차 풀이 완료 → 자동 재시도 ===')
            rsleep(1.0, 0.2)  # 캡차 처리 후 안정화
            return auto_shop(_retry_after_captcha=False)
        _shop_captcha_solved = False
        return False
...
# === 3층 맵 (red) 회수 루트 설정 =====================================
# 사냥은 2층(floor1_y_baseline=100)에서, 2분마다 회수 루트로 1/2/3층 다 훑어 펫이 줍게 함.
COLLECTION_INTERVAL = 100.0  # 100초
COLLECTION_TIMEOUT  = 90.0   # 회수 루트 전체 타임아웃 — 안 끝나면 강제 종료

FLOOR_Y_TOL    = 4    # Y 매칭 허용 오차
...
def _clear_mobs(x_min: int, x_max: int, duration: float = 3.0,
                attack_interval: float = 1.0) -> None:
    """X 범위 안에서 duration초 동안 공격 + 위치 유지 — 점프 전 몹 정리.
    범위 밖으로 밀리면 center 향해 walk로 복귀, 그 외엔 attack_interval마다 attack.
    attack_interval — 공격 간격(s). 기본 1.0 (~1회/초). 게임 attack cd보다 빠른 tap 씹힘 방지."""
    center = (x_min + x_max) // 2
    print(f'[clear] X={x_min}-{x_max} 몹 정리 ({duration:.1f}s, attack {attack_interval:.2f}s 간격)')
...
def _bounce_with_z_v2(x_min: int, x_max: int, round_trips: int = 2,
    ...
    2층(Y≈FLOOR2_Y)으로 떨어졌고 X가 zone 안이면 up_teleport 1회 복구 시도.
    ...
    try:
        ...
        while ends_hit < round_trips * 2 and time.time() - start < timeout and not STOP:
            ...
            if not pos:
                ...
                continue
            mm_x, mm_y = pos
            # 2층 떨어짐 + zone 안 → 몹 정리 1초 + up_teleport 복구 (1초 cooldown, 왕복 1회 카운트)
            if mm_y >= fall_y_threshold and x_min <= mm_x <= x_max and \
                    now - last_uptp_attempt > 1.0:
                ends_hit += 2
                print(f'[bounce-v2]   ⚠ 2층 떨어짐 mm={pos} → 몹 정리 1초 + up_teleport 복구 (ends_hit={ends_hit})')
                _clear_mobs(x_min, x_max, duration=1.0)
                up_teleport()
                rsleep(0.3, 0.2)
                tap(ATTACK_KEY, ATTACK_DOWN_SECONDS)
...
def _collect_3f_route() -> None:
    ...
    if STOP:
        return
    print(f'[3f-route] [A] hold_up_teleport(1.3) — Y={ROUTE_3F_TOP_Y} 도달까지 3회')
    top_reached = False
    for tp_try in range(3):
        ...
        if pos and pos[1] <= ROUTE_3F_TOP_Y + 5:
            ...
            top_reached = True
            break
        print(f'[3f-route] [A] 미도달 #{tp_try + 1}/3 mm={pos}')
    if not top_reached:
        print('[3f-route] [A] !! Y=102 미도달 → 종료')
        return

    # [B] 몹 정리 + 공격 1회 tap + drop_down → 3층 (Y=113 도달까지 최대 2회)
    print('[3f-route] [B] 몹 정리 + 공격 1회 + drop_down → 3층(Y=113)')
    _clear_mobs(ROUTE_3F_ENTRY_X - 5, ROUTE_3F_ENTRY_X + 5, duration=1.4)
    if STOP:
        return
    tap(ATTACK_KEY, ATTACK_DOWN_SECONDS)
    ...
    if pos and ROUTE_3F_PLATFORM_X[0] <= pos[0] <= ROUTE_3F_PLATFORM_X[1] and \
        ...
        _bounce_with_z_v2(ROUTE_3F_PLATFORM_X[0], ROUTE_3F_PLATFORM_X[1], round_trips=2)
    else:
    ...
    # [F] 2층 계단 z bounce 2회 — X 맞고 사냥라인보다 위면 platform 위로 간주
    pos = find_char_minimap_pos(grab())
    if pos and ROUTE_F2_STAIR_X[0] <= pos[0] <= ROUTE_F2_STAIR_X[1] and \
            pos[1] < FLOOR2_Y - 5:
        print(f'[3f-route] [F] 2층 계단 X 범위 도달 mm={pos} (Y<{FLOOR2_Y-5}) → z bounce 2회')
        _bounce_with_z_v2(ROUTE_F2_STAIR_X[0], ROUTE_F2_STAIR_X[1], round_trips=2)
    else:
...
def _collect_phase_f1_to_hunt(skip_patrol: bool = False):
    ...
    if not skip_patrol:
        # 시작 위치 확인 — 좌측 기준점(180)보다 왼쪽에 떨어졌으면 첫 [8a] 생략하고 바로 [8b]
        cur_pos = find_char_minimap_pos(grab())
        skip_8a_first = bool(cur_pos and cur_pos[0] < COLLECT_F1_LEFT_STOP_X)
        if skip_8a_first:
            print(f'[recover/collect] 시작 mm={cur_pos} → 좌측 기준({COLLECT_F1_LEFT_STOP_X}) 왼쪽 → 첫 [8a] 생략, 바로 [8b]')

        # 1층 좌↔우 왕복 2회 (펫이 따라올 시간 + 회수 확실히)
        # 첫 왕복: 우측 끝까지 (f1_hi). 두 번째 왕복: 우측 180까지만 (동선 단축)
        for trip in range(2):
            if STOP:
                return
            # [8a] 좌측 X=COLLECT_F1_LEFT_STOP_X 이동 — 펫 좌측 회수
            if trip == 0 and skip_8a_first:
                pass  # 첫 trip만 skip (이미 좌측 시작이면)
            else:
                print(f'[recover/collect] [8a] 1층 좌측 X={COLLECT_F1_LEFT_STOP_X} 이동 (왕복 {trip + 1}/2)')
                _route_to_x(COLLECT_F1_LEFT_STOP_X - 10, COLLECT_F1_LEFT_STOP_X + 5,
                            target_y=FLOOR1_Y, y_tol=15, timeout=12.0, walk_threshold=0)
                if STOP:
                    return

            # [8b] 우측 끝 이동 — 펫 우측 회수. 두 번째 왕복은 X=180까지만
            right_end = f1_hi if trip == 0 else 180
            print(f'[recover/collect] [8b] 1층 우측 X={right_end} 이동 (왕복 {trip + 1}/2)')
            _route_to_x(right_end - 15, right_end, target_y=FLOOR1_Y, y_tol=15, timeout=15.0,
                        walk_threshold=0)
            if STOP:
                return
    else:
    ...
    _clear_mobs(clear_lo, clear_hi, duration=1.0)
    for tp_try in range(3):
        if STOP:
            return
        ...
        rsleep(0.4, 0.2)
        pos = find_char_minimap_pos(grab())
        # 2층 도달 판정 — FLOOR2_Y(100) ±10 정확히, 그 위 platform(89/78)은 아직 도달 아님
        if pos and (FLOOR2_Y - 10) <= pos[1] <= (FLOOR2_Y + 10):
            print(f'[recover/collect] [10] 2층 도달 mm={pos} → 사냥 복귀 (zone 정렬 생략)')
            return
        # 위 platform(89 z hold / 78 최상층)에 박혀있으면 한 단계 떨어뜨리고 재시도
        if pos and pos[1] < FLOOR2_Y - 10:
            print(f'[recover/collect] [10] 위 platform mm={pos} → down_teleport로 사냥라인 복귀 시도')
            down_teleport()
            rsleep(0.5, 0.1)
            pos = find_char_minimap_pos(grab())
            if pos and (FLOOR2_Y - 10) <= pos[1] <= (FLOOR2_Y + 10):
                print(f'[recover/collect] [10] 2층 도달 (down_tp 후) mm={pos}')
                return
...
def collection_route():
    """간단 회수 루트 — 사냥 중 시간 경과 시 호출.
    [1] _drop_until_floor1 — 어디서든 1층까지 낙하
    [2] _collect_phase_f1_to_hunt — 1층 좌/우 패트롤 → up-tp로 사냥 zone 복귀"""
    ...
    print(f'[collect] === 끝 (소요 {time.time() - _collect_start_ts:.1f}s) ===')
...
def hunt():
    ...
    while not STOP:
        ...
        if mm_y is not None and floor1_y is not None and mm_y < floor1_y - FLOOR2_DELTA_Y:
            if on_floor2_since is None:
                ...
                if drop_attempts <= 3:
                    ...
                    # 10회 이상 — 모든 방법 시전, beep으로 사용자 알림 후 cycle 다시 (STOP 안 함)
                    try:
                        winsound.Beep(2500, 200)
                    except Exception:
                        pass
                    # cycle 리셋해서 drop_down부터 다시 시도
        ...
        if now - last_petfeed > PET_FEED_INTERVAL:
            print(f'[pet] 먹이 3개 시전 ({PET_FEED_KEY!r})')
            release_all()
            rsleep(0.25, 0.2)
...
def main():
    ...
    import signal
    globals()['wt'] = _wt  # _hotkey_listener_thread 안에서 wt.MSG() 참조
    pydirectinput.PAUSE = 0.02

    # SIGINT (Ctrl+C 또는 spurious 시그널) 무시 — 사냥 중 갑작스러운 중단 방지
    # 정지는 Ctrl+F12만 사용 (RegisterHotKey 기반, 시그널 무관)
    def _ignore_sigint(sig, frame):
        print('[SIGINT] 무시 — 정지는 Ctrl+F12 사용', flush=True)
    signal.signal(signal.SIGINT, _ignore_sigint)

    _threading.Thread(target=_hotkey_listener_thread, daemon=True).start()
